...
public class MyDBHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
        sqLiteDatabase.execSQL("CREATE TABLE `personal_data` ( `_personalid` INTEGER PRIMARY KEY AUTOINCREMENT , `name` TEXT, `gender` INTEGER, `birthday` TEXT , `cityname` INTEGER )");
        ...
        sqLiteDatabase.execSQL("CREATE TABLE `badydata` ( `_id` INTEGER PRIMARY KEY AUTOINCREMENT, `date` TEXT,`time` TEXT,`mothermilk` NUMERIC, `formula` NUMERIC, `weaning` NUMERIC, " +
                "`tall` NUMERIC, `weight` NUMERIC, `headlength` NUMERIC,`imagename` TEXT," +
